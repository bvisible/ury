...
function NewCustomerForm({ 
  ...
  return (
    <form className="space-y-4" onSubmit={handleAddCustomerSubmit}>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="new-customer-name">{__('Name')} <span className="text-red-500">*</span></label>
        <Input
        ...
        {formError && !newCustomerName && (
          <div className="text-xs text-red-500 mt-1">{__('Name is required')}</div>
        )}
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="new-customer-phone">{__('Phone')} <span className="text-red-500">*</span></label>
        ...
        {formError && !newCustomerPhone && (
          <div className="text-xs text-red-500 mt-1">{__('Phone is required')}</div>
        )}
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">{__('Customer Group')}</label>
        <Select
          placeholder={loadingGroups ? __('Loading...') : __('Select group')}
        ...
        {!loadingGroups && !customerGroups.length && (
          <div className="text-xs text-gray-400 mt-1">{__('No options')}</div>
        )}
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">{__('Territory')}</label>
        <Select
          placeholder={loadingTerritories ? __('Loading...') : __('Select territory')}
          ...
          {isOpen && (
            <div className="absolute w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
              {searchTerm.trim() === '' && !isSearching && !searchError && (
                <div className="p-4 text-center text-gray-400 text-sm select-none">{__('Please type to search...')}</div>
              )}
              {isSearching && (
              ...
              {!isSearching && !searchError && searchResults.length === 0 && searchTerm.trim() && (
                <div className="p-4 text-center text-gray-400 text-sm select-none">{__('No customers found')}</div>
              )}
      ...
      {showNewCustomerForm && (
        ...
        >
          <DialogContent className="w-full max-w-md p-4 max-h-[80vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{__('Add New Customer')}</h3>
            <NewCustomerForm 
